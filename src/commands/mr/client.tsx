...
type ProjectInfo = { id: number; name: string; pathWithNamespace: string; defaultBranch?: string };
...
type Action =
  | { type: "SET_INIT"; mode: "local" | "remote"; isGitRepo: boolean; cwd?: string; currentBranch?: string; localBranches?: string[]; detectedSource?: string; remoteUrl?: string; projectPath?: string }
  | { type: "SET_MODE"; mode: "local" | "remote" }
  | { type: "SET_CWD_INPUT"; input: string }
  | { type: "SET_CWD"; cwd: string; cwdError: string }
  | { type: "SET_LOADING"; loading: boolean }
  | { type: "SET_ERROR"; error: string }
  | { type: "SET_GIT_STATUS"; currentBranch: string; localBranches: string[]; detectedSource: string; remoteUrl: string; projectPath: string }
  ...
  | { type: "SELECT_REMOTE_PROJECT"; projectId: number; projectName: string; projectPath: string }
  ...
  | { type: "SET_MR_STATUS"; status: string; url?: string; iid?: number; error?: string; existing?: boolean }
...
const reducer = (state: State, action: Action): State => {
  switch (action.type) {
    ...
    case "SET_PROJECT_OPEN":
      return { ...state, projectOpen: action.open, projectSearch: "", projectIndex: -1 };
    ...
    case "SET_SOURCE_BRANCH_OPEN":
      return { ...state, sourceBranchOpen: action.open, sourceBranchSearch: "", sourceBranchIndex: -1 };
    case "SET_SOURCE_BRANCH_SEARCH":
      return { ...state, sourceBranchSearch: action.search, sourceBranchIndex: -1 };
    case "SET_SOURCE_BRANCH_INDEX":
      return { ...state, sourceBranchIndex: action.index };
    case "SELECT_SOURCE_BRANCH":
      return { ...state, sourceBranch: action.branch, sourceBranchOpen: false, step: state.step < 2 && state.targetBranch ? 2 : state.step };
    ...
    case "SELECT_TARGET_BRANCH":
      const branchStep = state.mode === "local"
      ...
      return { ...state, targetBranch: action.branch, branchOpen: false, step: branchStep };
    ...
    case "SET_PUSH_STATUS":
      const pushStep = action.status === "success" ? Math.max(state.step, 3) : state.step;
      return { ...state, pushStatus: action.status as State["pushStatus"], step: pushStep };
...
const MrClient: FC = () => {
  ...
  useEffect(() => {
    if (s.mode !== "remote" || s.projects.length > 0 || s.projectsLoading) return;
  ...
  useEffect(() => {
    if (!s.projectOpen && !s.sourceBranchOpen && !s.branchOpen && !s.memberOpen) return;
  ...
  const createMR = async () => {
    ...
    if (!sourceBranch) {
      d({ type: "SET_MR_STATUS", status: "error", error: `${t("web.mrFromBranch")} — ${t("web.mrSelectFrom")}` });
      return;
    }
    if (!s.targetBranch) {
      d({ type: "SET_MR_STATUS", status: "error", error: `${t("web.mrIntoBranch")} — ${t("web.mrSelectInto")}` });
      return;
    }
    if (!s.mrTitle) {
      d({ type: "SET_MR_STATUS", status: "error", error: `${t("web.mrTitleLabel")} — ${t("web.mrEnterTitle")}` });
      return;
    }
    ...
    try {
      ...
      const data = await res.json();
      if (data.error) {
        d({ type: "SET_MR_STATUS", status: "error", error: data.detail ? `${data.error}: ${data.detail}` : data.error });
        return;
      }
      d({
        type: "SET_MR_STATUS",
  ...
  const pipelineSteps =
    s.mode === "local"
      ? [
          { label: t("web.mrStepBranch"), done: s.step > 0, active: s.step === 0 },
          { label: t("web.mrStepTitle"), done: s.step > 1, active: s.step === 1 },
          { label: t("web.mrStepPush"), done: s.pushStatus === "success", active: s.step === 2 },
          { label: t("web.mrStepMR"), done: s.mrStatus === "success" || s.mrStatus === "existing", active: s.step === 3 },
          { label: t("web.mrStepJira"), done: s.ticketKeys.every((k) => s.jiraResults[k] === "success"), active: s.step === 4 },
        ]
      : [
          { label: t("web.mrStepProject"), done: s.projectId > 0, active: s.step === 0 },
          { label: t("web.mrStepBranch"), done: !!s.sourceBranch && !!s.targetBranch, active: s.step === 1 },
          { label: t("web.mrStepTitle"), done: s.step > 2, active: s.step === 2 },
          { label: t("web.mrStepMR"), done: s.mrStatus === "success" || s.mrStatus === "existing", active: s.step === 3 },
          { label: t("web.mrStepJira"), done: s.ticketKeys.every((k) => s.jiraResults[k] === "success"), active: s.step === 4 },
        ];
  ...
  return (
    <>
        ...
        <>
            ...
            <div class="cwd-section">
              <div class="cwd-row">
                <input
                  ...
                  value={s.cwdInput}
                  onInput={(e: Event) =>
                    d({ type: "SET_CWD_INPUT", input: (e.target as HTMLInputElement).value })
                  }
                />
                <button
                  class="cwd-btn"
          ...
          {s.currentBranch && (
            <>
                  ...
                  <div class="history-header">
                    ...
                    {s.clearConfirm ? (
                      <div style="display:flex;align-items:center;gap:8px">
                        <span style="font-size:12px;color:var(--text-3)">{t("web.clearConfirm")}</span>
                        <button class="history-clear" type="button" style="border-color:var(--error);color:var(--error)" onClick={clearHistory}>{t("web.clearHistory")}</button>
                        <button class="history-clear" type="button" onClick={() => d({ type: "CLEAR_CONFIRM_TOGGLE" })}>{t("web.cancel")}</button>
                      </div>
                    ) : (
                      <button class="history-clear" type="button" onClick={() => d({ type: "CLEAR_CONFIRM_TOGGLE" })}>{t("web.clearHistory")}</button>
                    )}
                  </div>
                  {s.history.map((entry: MrHistoryEntry) => (
                    <div class="history-item">
                      <div class="history-info">
                        <div class="history-title">{entry.title || `${entry.sourceBranch} → ${entry.targetBranch}`}</div>
                        <div class="history-branches">{entry.sourceBranch} → {entry.targetBranch}</div>
                        <div class="history-sub">{entry.projectName} · {entry.createdAt}</div>
                      </div>
                      <div class="history-actions">
                        <button class="history-exec-btn" type="button" onClick={() => executeHistory(entry)}>{t("web.quickExecute")}</button>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {s.quickExecuting && (
                <div class="loading-row"><span class="spinner" /> {t("web.mrCreatingMR")}</div>
              )}

              {(s.mrStatus === "success" || s.mrStatus === "existing") && !s.modalOpen && (
                <div class="result-card result-success">
                  <div class="result-label">{s.mrStatus === "existing" ? t("web.mrExistingNote") : t("web.mrResultUrl")}</div>
                  <div class="mr-url-row">
                    <a class="mr-url-link" href={s.mrUrl} target="_blank" rel="noopener">{s.mrUrl}</a>
                  <button class={`copy-btn${s.copied ? " copied" : ""}`} type="button" onClick={copyMrUrl}>{s.copied ? t("web.mrCopied") : t("web.mrCopy")}</button>
                  </div>
                </div>
              )}

              {s.history.length > 0 && !s.modalOpen && !s.quickExecuting && (
                <button class="new-mr-btn" type="button" onClick={() => d({ type: "TOGGLE_MODAL", open: true })}>{t("web.mrNewBtn")}</button>
              )}
              ...
              <div>
                <div class="result-card">
                  ...
                  {s.detectedSource && (
                    <span class="result-badge">{t("web.mrDetectSource")}: {s.detectedSource}</span>
                  )}
                </div>
                ...
                <div class="sel" style={`z-index:${s.branchOpen ? 100 : 1}`}>
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (!s.remoteBranches.length && s.projectId) loadRemoteBranches();
                    }}
                  >
                    <span class="sel-trigger-label">{t("web.mrIntoBranch")}</span>
                    ...
                    <div class="sel-dropdown" role="listbox">
                      <div class="sel-search">
                        <input
                          ...
                          onInput={(e: Event) =>
                            d({ type: "SET_BRANCH_SEARCH", search: (e.target as HTMLInputElement).value })
                          }
                          onKeyDown={(e: KeyboardEvent) => {
                            const n = selKeyDown(
                              e, s.branchOpen, fb.length, s.branchIndex,
                              (i) => { const b = fb[i]; if (b) d({ type: "SELECT_TARGET_BRANCH", branch: b.name }); },
                              () => openBranch(false),
                            );
                      ...
                      {s.remoteBranchesLoading ? (
                        <div class="sel-empty"><span class="spinner" /> {t("web.mrLoadingTarget")}</div>
                      ) : fb.length > 0 ? (
                        fb.map((b, i) => (
                          <div
                            class={`sel-item${b.name === s.targetBranch ? " active" : ""}${i === s.branchIndex ? " highlighted" : ""}`}
                            onMouseEnter={() => d({ type: "SET_BRANCH_INDEX", index: i })}
                            onClick={() => d({ type: "SELECT_TARGET_BRANCH", branch: b.name })}
                          >
                            <span class="sel-item-name">{b.name}</span>
                            {b.default && <span class="result-badge">{t("web.defaultBranch")}</span>}
                          </div>
                        ))
                      ) : (
                ...
                {s.projectId ? (
                  ...
                  <button class="action-btn secondary" type="button" onClick={resolveProject}>
              ...
              {s.targetBranch && (
                <div>
                  ...
                  <input class="title-input" type="text" placeholder={t("web.mrEnterTitle")} value={s.mrTitle}
                    onInput={(e: Event) => d({ type: "SET_MR_TITLE", title: (e.target as HTMLInputElement).value })} />
                  <div class="input-label">{t("web.mrDescriptionLabel")}</div>
                  <textarea class="desc-input" placeholder={t("web.mrEnterDescription")} value={s.mrDescription} rows={4}
                    onInput={(e: Event) => d({ type: "SET_MR_DESC", description: (e.target as HTMLInputElement).value })} />

                  {/* Reviewer selector */}
                  <div class="sel" style={`z-index:${s.memberOpen ? 100 : 1}`}>
                    <button class={`sel-trigger${s.memberOpen ? " open" : ""}`} type="button" role="combobox" aria-expanded={s.memberOpen}
                      onClick={() => { openMember(!s.memberOpen); if (!s.members.length && s.projectId) loadMembers(); }}>
                      <span class="sel-trigger-label">{t("web.mrReviewerLabel")}</span>
                      <span class={`sel-trigger-value${s.reviewerId ? "" : " empty"}`}>{s.reviewerId ? s.reviewerName : t("web.mrSkipReviewer")}</span>
                      <span class="sel-trigger-arrow">▼</span>
                    </button>
                    {s.memberOpen && (
                      <div class="sel-dropdown" role="listbox">
                        <div class="sel-search">
                          <input ref={memberSearchRef} class="sel-search-input" type="text" placeholder={t("web.mrFilterReviewer")} value={s.memberSearch}
                            onInput={(e: Event) => d({ type: "SET_MEMBER_SEARCH", search: (e.target as HTMLInputElement).value })}
                            onKeyDown={(e: KeyboardEvent) => {
                              const n = selKeyDown(e, s.memberOpen, fm.length, s.memberIndex,
                                (i) => { const m = fm[i]; if (m) d({ type: "SELECT_REVIEWER", id: m.id, name: `${m.name} (@${m.username})` }); },
                                () => openMember(false),
                              );
                              if (n !== undefined) d({ type: "SET_MEMBER_INDEX", index: n });
                            }} />
                        </div>
                        <div class={`sel-item${!s.reviewerId ? " active" : ""}`} onClick={() => d({ type: "SELECT_REVIEWER", id: 0, name: "" })}>
                          <span class="sel-item-name">{t("web.mrSkipReviewer")}</span>
                        </div>
                        {s.membersLoading ? (
                          <div class="sel-empty"><span class="spinner" /> {t("web.mrLoadingReviewer")}</div>
                        ) : fm.length > 0 ? (
                          fm.map((m, i) => (
                            <div class={`sel-item${m.id === s.reviewerId ? " active" : ""}${i === s.memberIndex ? " highlighted" : ""}`}
                              onMouseEnter={() => d({ type: "SET_MEMBER_INDEX", index: i })}
                              onClick={() => d({ type: "SELECT_REVIEWER", id: m.id, name: `${m.name} (@${m.username})` })}>
                  ...
                  {/* Draft toggle */}
                  <div class="draft-toggle" onClick={() => d({ type: "SET_DRAFT", draft: !s.draft })}>
                    <div class={`draft-toggle-check${s.draft ? " active" : ""}`}>{s.draft && "✓"}</div>
              ...
              {s.targetBranch && (
                <div>
                  {s.pushStatus === "idle" && (
                    <button class="action-btn" type="button" onClick={pushBranch}>{t("web.mrPushBtn")}</button>
                  )}
                  {s.pushStatus === "running" && (
                    <div class="loading-row"><span class="spinner" /> {t("web.mrPushing")}</div>
                  )}
                  {s.pushStatus === "success" && (
                    <div class="result-card result-success">
                      <div class="result-label">{t("web.mrPushBtn")}</div>
                      <div class="result-text success">{t("web.mrPushSuccess")}</div>
                    </div>
                  )}
                  {s.pushStatus === "error" && (
                    <div class="result-card result-error">
                      <div class="result-text error">{t("web.mrPushFailed")}</div>
                    </div>
                  )}
                  {s.pushStatus !== "success" && s.pushStatus !== "running" && (
                    <button class="action-btn secondary" type="button" onClick={() => d({ type: "SET_STEP", step: 3 })}>
                      {t("web.mrSkipPush")}
                    </button>
                  )}
                </div>
              ...
              {/* ── Step 4: Create MR ── */}
              {s.mrStatus === "idle" && (
                <div><button class="action-btn" type="button" onClick={createMR} disabled={!s.targetBranch || !s.mrTitle}>{s.draft ? t("web.mrCreateDraftBtn") : t("web.mrCreateBtn")}</button></div>
              )}
              {s.mrStatus === "running" && (
                <div class="loading-row"><span class="spinner" /> {t("web.mrCreatingMR")}</div>
              )}
              ...
              {s.mrStatus === "error" && (
                <div class="result-card result-error"><div class="result-text error">{s.mrError}</div></div>
              )}
              ...
              {s.mrUrl && s.ticketKeys.length > 0 && (
                <div>
                  {s.ticketKeys.map((key) => (
                    <div class="result-card">
                      <div class="result-label">{t("web.mrStepJira")}: {key}</div>
                      <button class="action-btn secondary" type="button"
                        onClick={() => addJiraComment(key, t("web.mrJiraCommentTemplate", { key, mrUrl: s.mrUrl }))}>
                        {t("web.mrJiraCommentBtn")}
                      </button>
                      <button class="action-btn secondary" type="button" onClick={() => loadJiraTransitions(key)}>
                        {t("web.mrJiraTransitionBtn")}
                      </button>
                      {s.jiraTransitions[key] && (
                        <div style="margin-top: 8px;">
                          {s.jiraTransitions[key].map((tr) => (
                            <button class="action-btn secondary" type="button" onClick={() => transitionJira(key, tr.id)} style="margin-bottom: 4px">{tr.name}</button>
                          ))}
                        </div>
                      )}
                      {s.jiraResults[key] === "success" && <span class="result-badge">{t("web.executeSuccess")}</span>}
                      {s.jiraResults[key] === "error" && <span style="color: var(--error); font-size: 12px">{t("web.executeFailed")}</span>}
                    </div>
                  ))}
                </div>
              )}
              {s.mrUrl && s.ticketKeys.length === 0 && (
                <div class="result-card"><div class="result-text">{t("web.mrNoTickets")}</div></div>
              )}
            </>
              )}
            </>
        ...
        <>
          ...
          <div class="sel" style={`z-index:${s.projectOpen ? 100 : 1}`}>
            ...
            >
              ...
              <span class={`sel-trigger-value${s.projectId ? "" : " empty"}`}>
              ...
              <div class="sel-dropdown" role="listbox">
                <div class="sel-search">
                  <input
                    ...
                    onInput={(e: Event) =>
                      d({ type: "SET_PROJECT_SEARCH", search: (e.target as HTMLInputElement).value })
                    }
                    onKeyDown={(e: KeyboardEvent) => {
                      const n = selKeyDown(
                        e, s.projectOpen, fp.length, s.projectIndex,
                        (i) => {
                          const p = fp[i];
                          if (p) {
                            d({ type: "SELECT_REMOTE_PROJECT", projectId: p.id, projectName: p.name, projectPath: p.pathWithNamespace });
                ...
                {s.projectsLoading ? (
                  <div class="sel-empty"><span class="spinner" /> {t("web.mrLoadingProjects")}</div>
                ) : fp.length > 0 ? (
                  fp.map((p, i) => (
                    <div
          ...
          {s.projectId > 0 && (
            <div>
              ...
              <div class="sel" style={`z-index:${s.sourceBranchOpen ? 200 : 1}`}>
                <button
                ...
                >
                  <span class="sel-trigger-label">{t("web.mrFromBranch")}</span>
                  <span class={`sel-trigger-value${s.sourceBranch ? "" : " empty"}`}>
                  ...
                  <div class="sel-dropdown" role="listbox">
                    <div class="sel-search">
                      <input
                        ...
                        onInput={(e: Event) =>
                          d({ type: "SET_SOURCE_BRANCH_SEARCH", search: (e.target as HTMLInputElement).value })
                        }
                        onKeyDown={(e: KeyboardEvent) => {
                          const n = selKeyDown(
                            e, s.sourceBranchOpen, fsb.length, s.sourceBranchIndex,
                            (i) => { const b = fsb[i]; if (b) d({ type: "SELECT_SOURCE_BRANCH", branch: b.name }); },
                            () => openSourceBranch(false),
                          );
                          if (n !== undefined) d({ type: "SET_SOURCE_BRANCH_INDEX", index: n });
                        }}
                      />
                    </div>
                    {s.remoteBranchesLoading ? (
                      ...
                      fsb.map((b, i) => (
                        <div
                          ...
                          onMouseEnter={() => d({ type: "SET_SOURCE_BRANCH_INDEX", index: i })}
                          onClick={() => d({ type: "SELECT_SOURCE_BRANCH", branch: b.name })}
                        >
                          <span class="sel-item-name">{b.name}</span>
                          {b.default && <span class="result-badge">{t("web.defaultBranch")}</span>}
                        </div>
          ...
          {/* ── Step 3: Create MR ── */}
          {s.mrStatus === "idle" && (
            <div><button class="action-btn" type="button" onClick={createMR} disabled={!s.sourceBranch || !s.targetBranch || !s.mrTitle}>{s.draft ? t("web.mrCreateDraftBtn") : t("web.mrCreateBtn")}</button></div>
          )}
          {s.mrStatus === "running" && (
            <div class="loading-row"><span class="spinner" /> {t("web.mrCreatingMR")}</div>
          )}
      ...
      {s.modalOpen && (
        <div class="modal-overlay" onClick={(e: Event) => { if ((e.target as HTMLElement).classList.contains("modal-overlay")) d({ type: "TOGGLE_MODAL", open: false }); }}>
          <div class="modal-box">
            <div class="modal-header">
              <h3>{t("web.mrNewModalTitle")}</h3>
              <button class="modal-close" type="button" onClick={() => d({ type: "TOGGLE_MODAL", open: false })}>×</button>
            </div>
            ...
            {s.mode === "local" && s.currentBranch && (
              <>
                ...
                <div>
                  ...
                  {/* Target branch selector */}
                  <div class="sel" style={`z-index:${s.branchOpen ? 100 : 1}`}>
                    <button class={`sel-trigger${s.branchOpen ? " open" : ""}`} type="button" role="combobox" aria-expanded={s.branchOpen}
                      onClick={() => { openBranch(!s.branchOpen); if (!s.remoteBranches.length && s.projectId) loadRemoteBranches(); }}>
                      <span class="sel-trigger-label">{t("web.mrIntoBranch")}</span>
                      <span class={`sel-trigger-value${s.targetBranch ? "" : " empty"}`}>{s.targetBranch || t("web.mrSelectInto")}</span>
                      <span class="sel-trigger-arrow">▼</span>
                    </button>
                    {s.branchOpen && (
                      <div class="sel-dropdown" role="listbox">
                        <div class="sel-search">
                          <input ref={branchSearchRef} class="sel-search-input" type="text" placeholder={t("web.mrFilterInto")} value={s.branchSearch}
                            onInput={(e: Event) => d({ type: "SET_BRANCH_SEARCH", search: (e.target as HTMLInputElement).value })}
                            onKeyDown={(e: KeyboardEvent) => {
                        ...
                        {s.remoteBranchesLoading ? (
                          ...
                          fb.map((b, i) => (
                            <div class={`sel-item${b.name === s.targetBranch ? " active" : ""}${i === s.branchIndex ? " highlighted" : ""}`}
                              onMouseEnter={() => d({ type: "SET_BRANCH_INDEX", index: i })}
                              onClick={() => d({ type: "SELECT_TARGET_BRANCH", branch: b.name })}>
                              <span class="sel-item-name">{b.name}</span>
                              {b.default && <span class="result-badge">{t("web.defaultBranch")}</span>}
            ...
            {s.mode === "remote" && (
              <>
                ...
                {/* Project selector */}
                <div class="sel" style={`z-index:${s.projectOpen ? 100 : 1}`}>
                  <button class={`sel-trigger${s.projectOpen ? " open" : ""}`} type="button" role="combobox" aria-expanded={s.projectOpen}
                    onClick={() => openProject(!s.projectOpen)}>
                    <span class="sel-trigger-label">{t("web.projectLabel")}</span>
                    <span class={`sel-trigger-value${s.projectId ? "" : " empty"}`}>{s.projectId ? s.projectName : t("web.mrSelectProject")}</span>
                    <span class="sel-trigger-arrow">▼</span>
                  </button>
                  {s.projectOpen && (
                    <div class="sel-dropdown" role="listbox">
                      <div class="sel-search">
                        <input ref={projectSearchRef} class="sel-search-input" type="text" placeholder={t("web.mrSearchProject")} value={s.projectSearch}
                          onInput={(e: Event) => d({ type: "SET_PROJECT_SEARCH", search: (e.target as HTMLInputElement).value })}
                          onKeyDown={(e: KeyboardEvent) => {
                            const n = selKeyDown(e, s.projectOpen, fp.length, s.projectIndex,
                              (i) => { const p = fp[i]; if (p) { d({ type: "SELECT_REMOTE_PROJECT", projectId: p.id, projectName: p.name, projectPath: p.pathWithNamespace }); loadRemoteBranches(p.id); loadMembers(); } },
                              () => openProject(false),
                            );
                            if (n !== undefined) d({ type: "SET_PROJECT_INDEX", index: n });
                          }} />
                      ...
                      {s.projectsLoading ? (
                        ...
                        fp.map((p, i) => (
                          <div class={`sel-item${p.id === s.projectId ? " active" : ""}${i === s.projectIndex ? " highlighted" : ""}`}
                            onMouseEnter={() => d({ type: "SET_PROJECT_INDEX", index: i })}
                            onClick={() => { d({ type: "SELECT_REMOTE_PROJECT", projectId: p.id, projectName: p.name, projectPath: p.pathWithNamespace }); loadRemoteBranches(p.id); loadMembers(); }}>
                            <div class="sel-item-name">{p.name}</div>
                            <div class="sel-item-sub">{p.pathWithNamespace}</div>
                          </div>
                        ))
                ...
                {s.projectId > 0 && (
                  <div>
                    {/* Source branch */}
                    <div class="sel" style={`z-index:${s.sourceBranchOpen ? 200 : 1}`}>
                      <button class={`sel-trigger${s.sourceBranchOpen ? " open" : ""}`} type="button" role="combobox" aria-expanded={s.sourceBranchOpen}
                        onClick={() => { openSourceBranch(!s.sourceBranchOpen); if (!s.remoteBranches.length) loadRemoteBranches(); }}>
                        <span class="sel-trigger-label">{t("web.mrFromBranch")}</span>
                        <span class={`sel-trigger-value${s.sourceBranch ? "" : " empty"}`}>{s.sourceBranch || t("web.mrSelectFrom")}</span>
                        <span class="sel-trigger-arrow">▼</span>
                      </button>
                      {s.sourceBranchOpen && (
                        <div class="sel-dropdown" role="listbox">
                          <div class="sel-search">
                            <input ref={sourceBranchSearchRef} class="sel-search-input" type="text" placeholder={t("web.mrFilterFrom")} value={s.sourceBranchSearch}
                              onInput={(e: Event) => d({ type: "SET_SOURCE_BRANCH_SEARCH", search: (e.target as HTMLInputElement).value })}
                              onKeyDown={(e: KeyboardEvent) => {
                                const n = selKeyDown(e, s.sourceBranchOpen, fsb.length, s.sourceBranchIndex,
                                  (i) => { const b = fsb[i]; if (b) d({ type: "SELECT_SOURCE_BRANCH", branch: b.name }); },
                                  () => openSourceBranch(false),
                                );
                                if (n !== undefined) d({ type: "SET_SOURCE_BRANCH_INDEX", index: n });
                          ...
                          {s.remoteBranchesLoading ? (
                            ...
                            fsb.map((b, i) => (
                              <div class={`sel-item${b.name === s.sourceBranch ? " active" : ""}${i === s.sourceBranchIndex ? " highlighted" : ""}`}
                                onMouseEnter={() => d({ type: "SET_SOURCE_BRANCH_INDEX", index: i })}
                                onClick={() => d({ type: "SELECT_SOURCE_BRANCH", branch: b.name })}>
                                <span class="sel-item-name">{b.name}</span>
                                {b.default && <span class="result-badge">{t("web.defaultBranch")}</span>}
                    ...
                    <div class="sel" style={`z-index:${s.branchOpen ? 100 : 1}`}>
                      <button class={`sel-trigger${s.branchOpen ? " open" : ""}`} type="button" role="combobox" aria-expanded={s.branchOpen}
                        onClick={() => { openBranch(!s.branchOpen); if (!s.remoteBranches.length) loadRemoteBranches(); }}>
                        <span class="sel-trigger-label">{t("web.mrIntoBranch")}</span>
                        <span class={`sel-trigger-value${s.targetBranch ? "" : " empty"}`}>{s.targetBranch || t("web.mrSelectInto")}</span>
                        <span class="sel-trigger-arrow">▼</span>
                      </button>
            ...
            {(s.mrStatus === "success" || s.mrStatus === "existing") && (
              <button class="action-btn secondary" type="button" onClick={() => { d({ type: "TOGGLE_MODAL", open: false }); loadHistory(); }}>
                {t("web.cancel")}
              </button>
            )}
          </div>
