...
type CrosshairSVGProps = CrosshairVisualProps & {
	animated?: boolean;
	outerAngle?: number;
	innerAngle?: number;
};
...
export function CrosshairSVG({
	...
	const outerStyle = animated
		? `transform-origin:${cx}px ${cy}px;animation:crosshair-cw ${outerSpeed}s linear infinite`
		: `transform-origin:${cx}px ${cy}px;transform:rotate(${outerAngle}deg)`;

	const innerStyle = animated
		? `transform-origin:${cx}px ${cy}px;animation:crosshair-ccw ${innerSpeed}s linear infinite`
		: `transform-origin:${cx}px ${cy}px;transform:rotate(${innerAngle}deg)`;
...
type CrosshairProps = CrosshairVisualProps & {
...
export function Crosshair({
	...
	return (
		<div ref={wrapperRef} aria-hidden="true" style={wrapperStyle}>
			<CrosshairSVG
				color={color}
				outerSize={outerSize}
				innerSize={innerSize}
...
export const crosshairCss = `
@keyframes crosshair-cw {
  from { transform: rotate(0deg); }
  to { transform: rotate(360deg); }
}

@keyframes crosshair-ccw {
  from { transform: rotate(0deg); }
  to { transform: rotate(-360deg); }
}
`;
