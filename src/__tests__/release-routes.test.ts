...
// Mock controllers
const { mockGitlabListProjects, mockGitlabListBranches, mockGitlabGetFile } = vi.hoisted(() => ({
	mockGitlabListProjects: vi.fn(),
	mockGitlabListBranches: vi.fn(),
	mockGitlabGetFile: vi.fn(),
}));

const { mockJiraSearch, mockJiraGetProjectVersions, mockJiraCreateVersion, mockJiraCreateIssue, mockJiraListProjectKeys } = vi.hoisted(() => ({
...
describe("Release routes – GET /release/", () => {
	it("redirects to /config when required keys missing", async () => {
		mockConfigGetConfig.mockReturnValue({ jiraHost: "", jiraName: "", jiraPassword: "", gitlabHost: "", gitlabKey: "" });
...
describe("Release routes – GET /release/api/config", () => {
	it("returns jiraHost and gitlabHost from config", async () => {
		...
		expect(data).toEqual({ jiraHost: "https://jira.com", gitlabHost: "https://gitlab.com" });
	});
...
describe("Release routes – GET /release/api/projects", () => {
	...
	it("returns error on GitlabController failure", async () => {
		mockGitlabListProjects.mockRejectedValueOnce(new Error("connection failed"));
		const res = await app.fetch(new Request("http://localhost/api/projects"));
...
describe("Release routes – GET /release/api/projects/:id/branches", () => {
	it("returns branches for a project", async () => {
		mockGitlabListBranches.mockResolvedValueOnce([{ name: "main", default: true }]);
		const res = await app.fetch(new Request("http://localhost/api/projects/5/branches"));
		expect(res.status).toBe(200);
		const data = await res.json();
	...
	it("returns error on failure", async () => {
		mockGitlabListBranches.mockRejectedValueOnce(new Error("project not found"));
		const res = await app.fetch(new Request("http://localhost/api/projects/999/branches"));
		expect(res.status).toBe(500);
		const data = await res.json();
...
describe("Release routes – GET /release/api/projects/:id/pom-version", () => {
	it("returns parsed POM version info", async () => {
		...
		const res = await app.fetch(new Request("http://localhost/api/projects/5/pom-version?ref=master"));
		expect(res.status).toBe(200);
		const data = await res.json();
		expect(data.version).toBe("1.0");
		expect(data.groupId).toBe("com.ex");
		expect(data.artifactId).toBe("app");
	});

	it("defaults ref to master when not specified", async () => {
		const pomXml = `<project><groupId>g</groupId><artifactId>a</artifactId><version>2.0</version></project>`;
		const base64 = Buffer.from(pomXml).toString("base64").replace(/\n/g, "");
		mockGitlabGetFile.mockResolvedValueOnce({ content: base64 });
	...
	it("returns error on failure", async () => {
		...
		const res = await app.fetch(new Request("http://localhost/api/projects/5/pom-version?ref=main"));
		expect(res.status).toBe(500);
		const data = await res.json();
...
describe("Release routes – POST /release/api/jira/search", () => {
	it("returns search results", async () => {
		mockJiraSearch.mockResolvedValueOnce({ total: 1, issues: [{ key: "PROJ-1" }] });
		const res = await app.fetch(new Request("http://localhost/api/jira/search", {
			method: "POST",
	...
	it("defaults maxResults to 5 when not provided", async () => {
		...
		await app.fetch(new Request("http://localhost/api/jira/search", {
			method: "POST",
...
describe("Release routes – POST /release/api/jira/ensure-version", () => {
	it("returns existing version when found", async () => {
		mockJiraGetProjectVersions.mockResolvedValueOnce([{ id: "v1", name: "1.0.0" }]);
		const res = await app.fetch(new Request("http://localhost/api/jira/ensure-version", {
			method: "POST",
...
describe("Release routes – POST /release/api/jira/create-issue", () => {
	it("creates issue and returns key", async () => {
		...
		const res = await app.fetch(new Request("http://localhost/api/jira/create-issue", {
			method: "POST",
			body: JSON.stringify({
				projectKey: "PROJ",
				summary: "Release 1.0",
...
describe("Release routes – GET /release/api/jira/projects", () => {
	it("returns project keys", async () => {
		mockJiraListProjectKeys.mockResolvedValueOnce([{ key: "PROJ", id: "10000" }]);
		const res = await app.fetch(new Request("http://localhost/api/jira/projects"));
		expect(res.status).toBe(200);
		const data = await res.json();
	...
	it("returns error on failure", async () => {
		mockJiraListProjectKeys.mockRejectedValueOnce(new Error("jira unavailable"));
		const res = await app.fetch(new Request("http://localhost/api/jira/projects"));
		expect(res.status).toBe(500);
	});
});