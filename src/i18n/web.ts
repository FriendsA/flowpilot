...
export function detectLocaleFromHeader(acceptLanguage: string): Locale {
  const langs = acceptLanguage.split(",").map((s) => (s.trim().split(";")[0] ?? "").toLowerCase());
  for (const lang of langs) {
    const normalized = lang === "zh" || lang.startsWith("zh-") ? "zh-CN" : lang.startsWith("en") ? "en" : lang;
    if (SUPPORTED.includes(normalized as Locale)) return normalized as Locale;
  }
  return "zh-CN";
}

export function detectLocaleFromCookie(cookie: string | undefined): Locale | null {
  if (!cookie) return null;
  const match = cookie.match(/(?:^|;\s*)locale=([^;]+)/);
  if (match?.[1] && SUPPORTED.includes(match[1] as Locale)) return match[1] as Locale;
  return null;
}
